...
public class RoleInfo
{
        ...
        bool suppressGhostInfo = false)
    {
        ...
        if (Lawyer.target != null && p.PlayerId == Lawyer.target.PlayerId &&
            CachedPlayer.LocalPlayer.Control != Lawyer.target)
        ...
        if (!suppressGhostInfo && p != null)
        {
            if (p == Shifter.shifter &&
                (CachedPlayer.LocalPlayer.Control == Shifter.shifter || Helpers.shouldShowGhostInfo()) &&
                Shifter.futureShift != null)
                roleName += Helpers.cs(Color.yellow, " ← " + Shifter.futureShift.Data.PlayerName);
            if (p == Vulture.vulture &&
                (CachedPlayer.LocalPlayer.Control == Vulture.vulture || Helpers.shouldShowGhostInfo()))
            ...
            if (Helpers.shouldShowGhostInfo())
            {
                ...
                if (p == Arsonist.arsonist)
                    roleName = roleName + Helpers.cs(Arsonist.color,
                        $" ({CachedPlayer.AllPlayers.Count(x => { return x.Control != Arsonist.arsonist && !x.Data.IsDead && !x.Data.Disconnected && !Arsonist.dousedPlayers.Any(y => y.PlayerId == x.PlayerId); })} left)");
