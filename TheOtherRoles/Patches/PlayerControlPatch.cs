...
public static class PlayerControlFixedUpdatePatch
{
                                           ...
                                           (isMorphedMorphling && Morphling.morphTarget == Medic.shielded)))
            {
                hasVisibleShield = Medic.showShielded == 0 || Helpers.shouldShowGhostInfo() // Everyone or Ghost info
                                                           || (Medic.showShielded == 1 &&
                                                               (CachedPlayer.LocalPlayer.Control == Medic.shielded ||
                ...
                hasVisibleShield = hasVisibleShield && (Medic.meetingAfterShielding || !Medic.showShieldAfterMeeting ||
                                                        CachedPlayer.LocalPlayer.Control == Medic.medic ||
        ...
        else
        {
            ...
            localPlayerPositions.Insert(0,
                new Tuple<Vector3, bool>(CachedPlayer.LocalPlayer.transform.position,
                    CachedPlayer.LocalPlayer.Control.CanMove)); // CanMove = CanMove
        }
    }
    ...
    private static void privateInvestigatorSetTarget()
    {
        if (PrivateInvestigator.privateInvestigator == null ||
            PrivateInvestigator.privateInvestigator != CachedPlayer.LocalPlayer.Control) return;
    ...
    private static void detectiveUpdateFootPrints()
    {
        ...
        if (Detective.timer <= 0f)
        {
            ...
            foreach (PlayerControl player in CachedPlayer.AllPlayers)
                if (player != null && player != CachedPlayer.LocalPlayer.Control && !player.Data.IsDead &&
                    !player.inVent)
    ...
    {
        if (CachedPlayer.LocalPlayer.Control == null ||
    ...
    {
        var jackalHighlight = Engineer.highlightForTeamJackal && (CachedPlayer.LocalPlayer.Control == Jackal.jackal ||
                                                                  CachedPlayer.LocalPlayer.Control ==
                                                                  Sidekick.sidekick);
    ...
    {
        if (!CachedPlayer.LocalPlayer.Data.Role.IsImpostor || !CachedPlayer.LocalPlayer.Control.CanMove ||
    ...
    private static void trackerUpdate()
    {
        ...
        if (Tracker.arrow?.arrow != null)
        {
            ...
            if (Tracker.tracker != null && Tracker.tracked != null &&
                CachedPlayer.LocalPlayer.Control == Tracker.tracker && !Tracker.tracker.Data.IsDead)
            {
        ...
        if (Tracker.tracker != null && Tracker.tracker == CachedPlayer.LocalPlayer.Control &&
    ...
    public static void updatePlayerInfo()
    {
        ...
        foreach (PlayerControl p in CachedPlayer.AllPlayers)
        {
            ...
            if (Snitch.snitch != null && Snitch.seeInMeeting && Snitch.canSeeRoles && !Snitch.snitch.Data.IsDead)
            {
                ...
                var completedSnitch = Snitch.seeInMeeting && CachedPlayer.LocalPlayer.Control == Snitch.snitch &&
                                      numberOfTasks == 0;
            ...
            if (snitchFlag ||
                (Lawyer.lawyerKnowsRole && CachedPlayer.LocalPlayer.Control == Lawyer.lawyer && p == Lawyer.target) ||
                p == CachedPlayer.LocalPlayer.Control || CachedPlayer.LocalPlayer.Data.IsDead ||
                (CachedPlayer.LocalPlayer.Control == Slueth.slueth &&
                ...
                (CachedPlayer.LocalPlayer.Control == Poucher.poucher &&
                 Poucher.killed.Any(x => x.PlayerId == p.PlayerId)))
            {
                ...
                var playerInfoText = "";
                var meetingInfoText = "";
                if (p == CachedPlayer.LocalPlayer.Control || (TORMapOptions.impostorSeeRoles && Spy.spy == null &&
                ...
                else if (TORMapOptions.ghostsSeeInformation)
                {
                    ...
                    meetingInfoText = playerInfoText;
                                                          ...
                                                          CachedPlayer.LocalPlayer.Control == Lawyer.lawyer &&
                                                          p == Lawyer.target))
                {
    ...
    {
        if (SecurityGuard.securityGuard == null || SecurityGuard.securityGuard != CachedPlayer.LocalPlayer.Control ||
    ...
    {
        if (SecurityGuard.securityGuard == null || CachedPlayer.LocalPlayer.Control != SecurityGuard.securityGuard ||
    ...
    {
        if (Vulture.vulture == null || CachedPlayer.LocalPlayer.Control != Vulture.vulture ||
    ...
    {
        if (Amnisiac.amnisiac == null || CachedPlayer.LocalPlayer.Control != Amnisiac.amnisiac ||
    ...
    {
        if (Radar.radar == null || CachedPlayer.LocalPlayer.Control != Radar.radar || Radar.localArrows == null ||
    ...
    {
        if (Medium.medium == null || Medium.medium != CachedPlayer.LocalPlayer.Control || Medium.medium.Data.IsDead ||
    ...
    {
        if (Swapper.swapper == null || CachedPlayer.LocalPlayer.Control != Swapper.swapper ||
    ...
    {
        if (Trapper.trapper == null || CachedPlayer.LocalPlayer.Control != Trapper.trapper ||
...
internal class BodyReportPatch
{
    private static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] GameData.PlayerInfo target)
    {
        ...
        var isMedicReport = Medic.medic != null && Medic.medic == CachedPlayer.LocalPlayer.Control &&
                            __instance.PlayerId == Medic.medic.PlayerId;
        var isDetectiveReport = Detective.detective != null &&
                                Detective.detective == CachedPlayer.LocalPlayer.Control &&
                                __instance.PlayerId == Detective.detective.PlayerId;
        var isSluethReport = Slueth.slueth != null && Slueth.slueth == CachedPlayer.LocalPlayer.Control &&
        ...
        if (isMedicReport || isDetectiveReport)
        {
            ...
            if (deadPlayer != null && deadPlayer.killerIfExisting != null)
            {
                ...
                if (!string.IsNullOrWhiteSpace(msg))
                {
                    ...
                    {
                        FastDestroyableSingleton<HudManager>.Instance.Chat.AddChat(CachedPlayer.LocalPlayer.Control,
                            msg);
...
public static class MurderPlayerPatch
{
    ...
    public static void Postfix(PlayerControl __instance, [HarmonyArgument(0)] PlayerControl target)
    {
        ...
        if (Sidekick.promotesToJackal && Sidekick.sidekick != null && !Sidekick.sidekick.Data.IsDead &&
            target == Jackal.jackal && Jackal.jackal == CachedPlayer.LocalPlayer.Control)
        {
            var writer = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.Control.NetId,
                (byte)CustomRPC.SidekickPromotes, SendOption.Reliable);
            AmongUsClient.Instance.FinishRpcImmediately(writer);
        ...
        if (target == Lawyer.target && AmongUsClient.Instance.AmHost && Lawyer.lawyer != null)
        {
            ...
            if (Undertaker.undertaker != null && CachedPlayer.LocalPlayer.Control == Undertaker.undertaker &&
        ...
        if (Seer.seer != null && (CachedPlayer.LocalPlayer.Control == Seer.seer || Helpers.shouldShowGhostInfo()) &&
        ...
        if (BountyHunter.bountyHunter != null && CachedPlayer.LocalPlayer.Control == BountyHunter.bountyHunter &&
        ...
        // Cleaner Button Sync
        if (Cleaner.cleaner != null && CachedPlayer.LocalPlayer.Control == Cleaner.cleaner &&
        ...
        // Witch Button Sync
        if (Witch.triggerBothCooldowns && Witch.witch != null && CachedPlayer.LocalPlayer.Control == Witch.witch &&
        ...
        // Warlock Button Sync
        if (Warlock.warlock != null && CachedPlayer.LocalPlayer.Control == Warlock.warlock &&
        ...
        // Ninja Button Sync
        if (Ninja.ninja != null && CachedPlayer.LocalPlayer.Control == Ninja.ninja && __instance == Ninja.ninja &&
        ...
        if (Bait.bait.FindAll(x => x.PlayerId == target.PlayerId).Count > 0)
        {
            ...
            if (Bait.showKillFlash && __instance == CachedPlayer.LocalPlayer.Control)
...
public static class ExilePlayerPatch
{
    public static void Postfix(PlayerControl __instance)
    {
        ...
        if (Sidekick.promotesToJackal && Sidekick.sidekick != null && !Sidekick.sidekick.Data.IsDead &&
            __instance == Jackal.jackal && Jackal.jackal == CachedPlayer.LocalPlayer.Control)
        {
            var writer = AmongUsClient.Instance.StartRpcImmediately(CachedPlayer.LocalPlayer.Control.NetId,
                (byte)CustomRPC.SidekickPromotes, SendOption.Reliable);
            AmongUsClient.Instance.FinishRpcImmediately(writer);
